...
module RripRedux
module Gtk3
  # GtkMultipleMusicBraizHits handles the window while displaying the dialog
  ...
  #
  class GtkMultipleMusicBrainzHits
    ...
    def initialize(metadata, main_instance)
      ...
      @image1 = Gtk::Image.new(stock: gDIALOG_QUESTION, size: gDIALOG)
      ...
      @image2 = Gtk::Image.new(stock: gOK, size: gLARGE_TOOLBAR)
  ...
  private
    ...
    def create_release_info_table
      ...
      release_info_table.tap do |t|
        t.attach(title_label,     0, 1, 0, 1, gFILL, gSHRINK,  0, 0)
        t.attach(title_value,     1, 2, 0, 1, gFILL, gSHRINK, 10, 0)
        t.attach(date_label,      0, 1, 1, 2, gFILL, gSHRINK,  0, 0)
        t.attach(date_value,      1, 2, 1, 2, gFILL, gSHRINK, 10, 0)
        t.attach(desc_label,      0, 1, 2, 3, gFILL, gSHRINK,  0, 0)
        t.attach(desc_value,      1, 2, 2, 3, gFILL, gSHRINK, 10, 0)
        t.attach(country_label,   0, 1, 3, 4, gFILL, gSHRINK,  0, 0)
        t.attach(country_value,   1, 2, 3, 4, gFILL, gSHRINK, 10, 0)
        t.attach(packaging_label, 0, 1, 4, 5, gFILL, gSHRINK,  0, 0)
        t.attach(packaging_value, 1, 2, 4, 5, gFILL, gSHRINK, 10, 0)
        t.attach(status_label,    0, 1, 5, 6, gFILL, gSHRINK,  0, 0)
        t.attach(status_value,    1, 2, 5, 6, gFILL, gSHRINK, 10, 0)
        t.attach(barcode_label,   0, 1, 6, 7, gFILL, gSHRINK,  0, 0)
        t.attach(barcode_value,   1, 2, 6, 7, gFILL, gSHRINK, 10, 0)
      end
    end
